def ExtractInstance( data, shifts, addresses, D, T, nof_decimals=2 ):
    ...
    orders = data[these & ((data.ACTION =='pickup') | (data.ACTION =='deliver')) ][['ACTION','ORDER','FINISH_LATITUDE','FINISH_LONGITUDE','EARLIEST_START_TIME','LATEST_START_TIME','LOAD_LM','LOAD_KG']].copy().reset_index(drop=True)
